...
// ---------------------------------------------------------------------------
// Upload mode (checkpoint-to-storage): NIXL_WRITE host buffers -> S3.
//
// Mirror of the download path (preparePrivateRanges + post/poll) but with
// NIXL_READ -> NIXL_WRITE and a pre-filled source buffer. Used to push CUDA
// checkpoint device-memory dumps (already staged to host files by the
// checkpointer) up to S3 over the same NIXL OBJ backend the restore path uses.
//
// Manifest format (one entry per line, '#' comments ignored):
//     <localpath> <bucket> <key>
// All entries must share one bucket (NIXL OBJ constraint).
// ---------------------------------------------------------------------------
struct UploadEntry { std::string path, bucket, key; uint64_t size; void *addr; };

static int uploadMain(const char *manifestPath) {
    std::ifstream in(manifestPath);
    if (!in) { std::fprintf(stderr, "upload: cannot open manifest %s: %s\n", manifestPath, std::strerror(errno)); return 1; }
    std::vector<UploadEntry> es;
    std::string line;
    while (std::getline(in, line)) {
        if (line.empty() || line[0] == '#') continue;
        std::istringstream ss(line);
        UploadEntry e{}; e.addr = nullptr;
        if (!(ss >> e.path >> e.bucket >> e.key)) continue;
        int fd = ::open(e.path.c_str(), O_RDONLY);
        if (fd < 0) { std::fprintf(stderr, "upload: open %s: %s\n", e.path.c_str(), std::strerror(errno)); return 1; }
        off_t sz = ::lseek(fd, 0, SEEK_END);
        if (sz <= 0) { std::fprintf(stderr, "upload: %s empty/lseek fail\n", e.path.c_str()); ::close(fd); return 1; }
        e.size = (uint64_t)sz;
        e.addr = ::mmap(nullptr, e.size, PROT_READ, MAP_SHARED, fd, 0);
        ::close(fd);
        if (e.addr == MAP_FAILED) { std::fprintf(stderr, "upload: mmap %s: %s\n", e.path.c_str(), std::strerror(errno)); return 1; }
        es.push_back(std::move(e));
    }
    if (es.empty()) { std::fprintf(stderr, "upload: manifest has no entries\n"); return 1; }
    const std::string bucket = es[0].bucket;
    for (const auto &e : es) if (e.bucket != bucket) { std::fprintf(stderr, "upload: mixed buckets not supported\n"); return 1; }
    ...
    std::vector<std::unique_ptr<nixlAgent>> agents;
    std::vector<nixlBackendH *> backends;
    if (!setupNixlObjAgents(1, bucket, throughput, agents, backends)) return 1;
    nixl_opt_args_t hint; hint.backends.push_back(backends[0]);

    // Register all source buffers (DRAM_SEG) + destination objects (OBJ_SEG).
    nixl_reg_dlist_t dram_all(DRAM_SEG), obj_all(OBJ_SEG);
    for (size_t i = 0; i < es.size(); ++i) {
        nixlBlobDesc d{}; d.addr = (uintptr_t)es[i].addr; d.len = es[i].size; d.devId = (uint32_t)i; dram_all.addDesc(d);
        nixlBlobDesc o{}; o.addr = 0; o.len = es[i].size; o.devId = (uint32_t)i; o.metaInfo = es[i].key; obj_all.addDesc(o);
    }
    if (agents[0]->registerMem(dram_all, &hint) != NIXL_SUCCESS ||
        agents[0]->registerMem(obj_all, &hint) != NIXL_SUCCESS) {
        std::fprintf(stderr, "upload: registerMem failed\n"); return 1;
    }

    uint64_t total = 0;
    auto t0 = std::chrono::steady_clock::now();
    for (size_t i = 0; i < es.size(); ++i) {
        nixl_reg_dlist_t d1(DRAM_SEG), o1(OBJ_SEG);
        nixlBlobDesc d{}; d.addr = (uintptr_t)es[i].addr; d.len = es[i].size; d.devId = (uint32_t)i; d1.addDesc(d);
        nixlBlobDesc o{}; o.addr = 0; o.len = es[i].size; o.devId = (uint32_t)i; o.metaInfo = es[i].key; o1.addDesc(o);
        nixl_xfer_dlist_t dx = d1.trim(), ox = o1.trim();
        nixlXferReqH *req = nullptr;
        nixl_status_t s = agents[0]->createXferReq(NIXL_WRITE, dx, ox, streamerAgentName(0), req, &hint);
        if (s != NIXL_SUCCESS || !req) { std::fprintf(stderr, "upload: createXferReq %s failed (%d)\n", es[i].key.c_str(), (int)s); return 1; }
        s = agents[0]->postXferReq(req, &hint);
        if (s < 0) { std::fprintf(stderr, "upload: postXferReq %s failed (%d)\n", es[i].key.c_str(), (int)s); return 1; }
        while (s != NIXL_SUCCESS) {
            s = agents[0]->getXferStatus(req);
            if (s < 0) { std::fprintf(stderr, "upload: xfer %s failed (%d)\n", es[i].key.c_str(), (int)s); return 1; }
        }
        agents[0]->releaseXferReq(req);
        total += es[i].size;
        std::fprintf(stderr, "[upload] %s -> s3://%s/%s (%.2f MiB) OK\n", es[i].path.c_str(), bucket.c_str(), es[i].key.c_str(), es[i].size / 1048576.0);
    }
    double dt = std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count();
    std::fprintf(stderr, "[upload] wrote %.2f MiB to s3://%s in %.3fs (%.2f GiB/s)\n",
                 total / 1048576.0, bucket.c_str(), dt, total / 1073741824.0 / (dt > 0 ? dt : 1));
    return 0;
}
...
static int run_main(int argc, char **argv) {
    ...
    static option longopts[] = {
        ...
        {"upload", no_argument, nullptr, 'u'},
        {nullptr, 0, nullptr, 0},
    };
    int opt;
    while ((opt = getopt_long(argc, argv, "m:u", longopts, nullptr)) != -1) {
        switch (opt) {
        case 'm':
        ...
        case 'u':
            ...
            break;
        default:
            std::fprintf(stderr, "Usage: %s --manifest <path> [--upload]\n", argv[0]);
            return 2;
        }
    }
    if (manifest_path == nullptr) manifest_path = std::getenv("CRIU_STREAMER_MANIFEST");
    if (!upload && std::getenv("CRIU_STREAMER_UPLOAD_MANIFEST")) {
        upload = true;
        manifest_path = std::getenv("CRIU_STREAMER_UPLOAD_MANIFEST");
    }
    ...
    if (upload) return uploadMain(manifest_path);
